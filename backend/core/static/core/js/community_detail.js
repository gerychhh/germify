(function () {
  ...
  domReady(function () {
    ...
    if (settingsForm && settingsUrl) {
      const previewName = document.getElementById('appearanceName');
      const previewDesc = document.getElementById('appearanceDesc');
      const previewCover = document.getElementById('appearanceCover');
      const previewAvatar = document.getElementById('appearanceAvatar');
      const nameInput = settingsForm.querySelector('input[name="name"]');
      const descInput = settingsForm.querySelector('textarea[name="description"]');
      ...
      function bindLivePreview() {
        if (nameInput && previewName) {
          nameInput.addEventListener('input', () => {
            previewName.textContent = nameInput.value || 'Название сообщества';
          });
        }
        if (descInput && previewDesc) {
          descInput.addEventListener('input', () => {
            previewDesc.textContent = descInput.value || 'Короткое описание';
          });
        }
        const renderFile = (input, target, styleKey) => {
          ...
          input.addEventListener('change', () => {
            ...
            reader.onload = (ev) => {
              if (styleKey === 'cover') {
                ...
                target.innerHTML = `<img src="${ev.target.result}" alt="">`;
              }
            };
            reader.readAsDataURL(file);
          });
        };
        renderFile(coverInput, previewCover, 'cover');
        renderFile(iconInput, previewAvatar, 'icon');
      }
