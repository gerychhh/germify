...
def community_detail(request, slug):
    ...
    can_manage_members = bool(membership and membership.has_permission("manage_members"))
    can_manage_requests = bool(membership and membership.has_permission("manage_requests"))
    can_manage_settings = bool(membership and membership.has_permission("manage_settings"))
    can_manage_appearance = bool(membership and membership.has_permission("manage_appearance"))
    can_manage_posts = bool(membership and membership.has_permission("manage_posts"))
    is_admin = can_manage_members or can_manage_settings or can_manage_appearance or can_manage_posts
    ...
    return render(request, "core/community_detail.html", {
        ...
        "can_manage_appearance": can_manage_appearance,
...
def community_settings_api(request, slug):
    ...
    if not membership or not (
        membership.has_permission("manage_settings")
        or membership.has_permission("manage_appearance")
        or membership.has_permission("manage_members")
    ):
        return JsonResponse({"error": "forbidden"}, status=403)
    ...
    if request.method == "GET":
        data = {
            ...
            "moderator_permissions": community.moderator_permissions,
        }
    ...
    if form.is_valid():
        form.save()
        community.moderator_permissions = form.cleaned_moderator_permissions()
        community.save(update_fields=["moderator_permissions"])
...
def community_member_role(request, slug, user_id):
    ...
    if not me or not me.has_permission("manage_members"):
        return JsonResponse({"error": "forbidden"}, status=403)
    if request.method != "POST":
...
def community_join_requests(request, slug):
    community = get_object_or_404(Community, slug=slug)
    me = CommunityMembership.objects.filter(community=community, user=request.user).first()
    if not me or not me.has_permission("manage_requests"):
        return JsonResponse({"error": "forbidden"}, status=403)
